describe "integration" do
  ...
  let(:start_command) { %w(bundle exec bunto serve).concat(args) }
  let(:stop_command) { ["pkill", "-f", "bunto"] }
  ...
  context "Bunto site" do
    let(:path) { "/" }

    it "serves the Bunto site", :skip => Gem.win_platform? do
  ...
  context "Admin site" do
    ...
    it "serves the Bunto site", :skip => Gem.win_platform? do
      with_index_stubbed do
        expect(response.code).to eql("200")
        expect(response.body).to match("Bunto Admin")
      end
    end
  end
  ...
  context "watching" do
    it "Bunto isn't watching", :skip => Gem.win_platform? do
