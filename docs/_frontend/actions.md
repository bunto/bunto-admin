...
Async action for fetching an object comprised of Bunto project configuration (from `_config.yml` by default)

### `putConfig(config)`

Async action for updating Bunto project configuration, after ensuring the content is not empty.
